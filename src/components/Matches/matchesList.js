...
class MatchesList extends Component {
    ...
    render() {
        return ( 
            <div>
                {
                    this.state.matchesList && this.state.matchesList.length > 0 ?
                        <NodeGroup data = {this.state.matchesList} keyAccessor = {(d)=>d.id} start = {()=>({
                            opacity: 0,
                            x: -200
                        })} enter = {(d, i)=> ({
                            opacity: [1],
                            x: [0],
                            timing: {duration: 500, delay: i * 50, ease: easePoly}
                        })} update = {(d, i)=> ({
                            opacity: [1],
                            x: [0],
                            timing: {duration: 500, delay: i * 50, ease: easePoly}
